...
| Поле             | Тип                                                  | Описание                                           |
|------------------|------------------------------------------------------|----------------------------------------------------|
| friend_status_id | INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY | Автоматически генерируемый ID                      |
| name             | VARCHAR UNIQUE NOT NULL                              | Название статуса уникальное и не может быть пустым |
...
| friend_status_id | INTEGER NOT NULL REFERENCES friend_status (friend_status_id)    | Ссылка на таблицу friend_status |
